...
class IonStructureViewElement(private val element: PsiElement) : StructureViewTreeElement, SortableTreeElement {
    ...
    override fun getChildren(): Array<IonStructureViewElement> =
        when (element) {
            ...
            is IonExpression -> listOfNotNull(
                listOfNotNull(element.expressionOperator?.let { IonStructureViewElement(it) }),
                element.atoms?.valueList?.getViewElementChildren()
            ).flatten().toTypedArray()

            is IonPair -> arrayOf(element.value.`object`?.struct?.let { IonStructureViewElement(it) } ?: IonStructureViewElement(element.value))

            is IonValue -> listOfNotNull(element.`object`?.let { IonStructureViewElement(IonPsiUtil.getPsiElement(it)!!) }).toTypedArray()
    ...
    private fun List<IonValue>.getViewElementChildren(): List<IonStructureViewElement> =
        map { value ->
            value.`object`?.let { IonStructureViewElement(IonPsiUtil.getPsiElement(it)!!) }
